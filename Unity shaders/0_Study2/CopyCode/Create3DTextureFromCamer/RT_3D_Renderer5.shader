Shader "Unlit/RT_3D_Renderer5"
{
    Properties
    {
        _Debug ("Debug", Range(0,1)) = 1
        _DebugSphere ("DebugSphere", Vector) = (0,0,0,0)
        _DebugBox ("DebugBox", Vector) = (0,0,0,0)
        _DebugBox2 ("DebugBox2", Vector) = (0,0,0,0)
        _DebugBox2Origin ("_DebugBox2Origin", Vector) = (0,0,0,0)
        _StarsTex ("Stars 2D Texture", 2D) = "white" {}
        _StarsTexRatio ("StarsTexRatio", Vector) = (1,1,1,1)
        _StarsScale ("StarsScale", Float) = 1.0
        _StarsScroll ("StarsScroll", Vector) = (0,0,0,0)
        _StarsIntensity ("StarsIntensity", Range(0,1)) = 1.0
        _StarsThreshold ("StarsThreshold", Range(0,1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _StarsTex;
            float4 _StarsScroll;
            float _StarsScale, _StarsIntensity, _StarsThreshold;
            float4 _VectorTest;
            float4 _Box;
            ...
            float4 _VolumeTexRatio, _DebugSphere, _StarsTexRatio, _DebugBox, _DebugBox2, _DebugBox2Origin;
            ...
            float getStarTex(float3 p){
                return tex2D(_StarsTex, (p.xz+_Time.y*_StarsScroll.xy)*_StarsScale) * _StarsIntensity;
            }

            float getNoisetoMap(float3 pos){
                float noise = tex3D(_NoiseTex, (pos * _StarsScale) * float3(_StarsTexRatio.xyz) + _StarsTexRatio.xyz) * getStarTex(pos) / _DebugBox.w;
                float box = sdBox(pos, _DebugBox.xyz);
                return max(noise, box);
            }
            ...
            float DE(float3 p, float3 worldPos, float mult)
            {
                //float d = getTex3D(p);
                float d = mult * max(tex3D(_VolumeTex, (p * _CloudScale) * float3(_VolumeTexRatio.xyz) + _VectorTest.xyz),sdBox(p+ _DebugBox2Origin.xyz, _DebugBox2.xyz));
                float d2 = mult * (length(p - _DebugSphere.xyz) - _DebugSphere.w) / (length(worldPos - _DebugSphere.xyz) - _DebugSphere.w);
                float oped = opUS(d,d2, _BlendFactor);
                return opUS(oped, clamp(getNoisetoMap(p),0.01,1), _BlendFactorLine1);
                //return opUS(oped, DELines(p, worldPos, 1.0), _BlendFactorStoL);
            }

            float DElight(float3 p, float3 worldPos)
            {
                float d = max(tex3D(_VolumeTex, (p * _CloudScale) * float3(_VolumeTexRatio.xyz) + _VectorTest.xyz),sdBox(p+ _DebugBox2Origin.xyz, _DebugBox2.xyz));
                float d2 = (length(p - _DebugSphere.xyz) - _DebugSphere.w) / (length(worldPos - _DebugSphere.xyz) - _DebugSphere.w);
                float oped = min(d,d2);
                return min(oped, getNoisetoMap(p));
                //return opUS(oped, DELines(p, worldPos, 1.0), _BlendFactorStoL);
            }
            ...
            float3 GetNormal(float3 p, float3 worldPos) {
                ...
                float3 n = float3(
                    DElight(p + e.xyy, worldPos) - DElight(p - e.xyy, worldPos),
                    DElight(p + e.yxy, worldPos) - DElight(p - e.yxy, worldPos),
                    DElight(p + e.yyx, worldPos) - DElight(p - e.yyx, worldPos));
                return normalize(n);
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float originalSphere = length(worldPos - _DebugSphere.xyz) - _DebugSphere.w;
                [unroll(100)]
                for(int i = 0; i < _MaxIter; i++)
                {
                    pos = worldPos + rd * factorDist;
                    float noise = pow(clamp((tex3D(_NoiseTex, (pos * _NoiseScale) + _VectorTest.xyz)), 0,1.0),2);
                    //tex3dval =  clamp((max(tex3D(_VolumeTex, (pos * _CloudScale) * float3(_VolumeTexRatio.xyz) + _VectorTest.xyz),sdBox(pos+ _DebugBox2Origin.xyz, _DebugBox2.xyz)) * (1-noise)),0,1.0);
                    //float sphere1 = (1-noise) * clamp((length(pos - _DebugSphere.xyz) - _DebugSphere.w) / (originalSphere), 0.01, 1);
                    //float lineOp = DELines(pos, worldPos, (1-noise));
                    //tex3dval = opUS(tex3dval, sphere1, _BlendFactor);
                    //tex3dval = opUS(tex3dval, clamp(getNoisetoMap(pos),0.01,1), _BlendFactorLine1);
                    tex3dval = DE(pos, worldPos, (1-noise));
