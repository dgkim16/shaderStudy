Shader "Unlit/RT_3D_Renderer4"
{
    Properties
    {
        _DebugSphere ("DebugSphere", Vector) = (0,0,0,0)
        _BlendFactor ("BlendFactorShape", Float) = 0.5
        _BlendFactorStoL ("BlendFactorShapeToLine", Float) = 0.5
        _BlendFactorColor ("BlendFactorColor", Float) = 0.5

        _LineSphere1 ("LineSphere1", Vector) = (0,0,0,0)
        _BlendFactorLine1 ("BlendFactor1", Float) = 0.5
        _LineSphere2 ("LineSphere2", Vector) = (0,0,0,0)
        _BlendFactorLine2 ("BlendFactor2", Float) = 0.5
        _LineSphere3 ("LineSphere3", Vector) = (0,0,0,0)
        _BlendFactorLine3 ("BlendFactor3", Float) = 0.5
        ...
        _SurfEdgeStrength("SurfEdgeStrength", Float) = 4.0
        _SurfMin("SurfMinTransparency", Float) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _MaxDist, _MaxIter, _BlendFactor, _SurfEdgeStrength, _SurfMin;
            float4 _EdgeColor;
            ...
            float DE(float3 p)
            {
                //float d = getTex3D(p);
                //float d = getTex3D(p);
                float d2 = length(p - _DebugSphere.xyz) - _DebugSphere.w;
                float d3 = length(p - _LineSphere1.xyz) - _LineSphere1.w;
                float d4 = length(p - _LineSphere2.xyz) - _LineSphere2.w;
                float op = opUS(d3, d4, _BlendFactorLine1);
                float op2 = opUS(d2, op, _BlendFactorStoL);
                return op2;
            }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                for(int i = 0; i < _MaxIter; i++)
                {
                    ...
                    float tex3dval = DE(pos);
                    hardLine = tex3dval * (1-noise);
                    if(hardLine < _Threshold + _CloudThreshold) {
                        retColor.rgb = float3(_Color.rgb * ((sin(hardLine * _ColorScale) * 0.8) + 0.8) / 0.8 + 0.1);
                        ...
                        if(hardLine <= _Threshold - _CloudThreshold * 0.5)
                        {
                            retColor.a = clamp(_SurfEdgeStrength * (retColor.a + _SurfMin), 0, 10);
                            if (retColor.a > _OpacityLimit)
                                retColor.a += _EdgeColor.a + _EdgeColor.r + _EdgeColor.g + _EdgeColor.b ;
                            retColor.rgb /= 2;
                            return retColor;
